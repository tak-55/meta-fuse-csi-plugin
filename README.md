...
`proxy/s3fs` の external test では、`s3.env` の `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` から一時的な `passwd_file` を生成し、その認証情報を `s3fs` に明示的に渡します。非 root の restricted Pod で `/data` を読みやすくするため、`s3fs` には既定で `-o uid=1000 -o gid=1000 -o umask=007` も渡します。
